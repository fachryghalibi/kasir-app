...
@section('title', 'Tambah Kategori Cash Flow')

@section('page-title', 'Tambah Kategori Cash Flow')
@section('page-description', 'Buat kategori pemasukan atau pengeluaran baru')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Form Kategori Baru</h3>
        </div>

        <form action="{{ route('boss.cash-flow-categories.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                    ...
                    placeholder="Contoh: Listrik, Gaji Karyawan, Modal Tambahan">
                @error('name')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
